...
<link rel="stylesheet" href="<?php echo MUNIO_SHORTCODES_DIR_URL; ?>/include/css/font-awesome.min.css" />
<link rel="stylesheet" href="<?php echo MUNIO_SHORTCODES_DIR_URL; ?>/include/css/shortcodes_popup.css" />

<script src="<?php echo MUNIO_SHORTCODES_DIR_URL; ?>/include/js/simple-slider.min.js"></script>
<script src="<?php echo MUNIO_SHORTCODES_DIR_URL; ?>/include/js/shortcodes_popup.js"></script>
...
function shortcode_options( $sc_name, $shortcode ){
    ...
    if( $shortcode ){
        ...
        if( isset($shortcode['require_icon']) && $shortcode['require_icon'] ){
            ...
            require_once( MUNIO_SHORTCODES_DIR . "/include/fontawesome-icons.php" );
        }
        ...
        if( isset( $shortcode['attributes'] ) && $shortcode['attributes'] ){
            ...
            foreach( $attributes as $attribute => $attr_options ){
                ...
                if( $attr_options['type'] == "icon" ){
                    ...
                    echo '<input class="attr" type="text" data-attrname="'.$attribute.'" value="' . $attr_options['default'] . '" />';
                    echo '<a class="btn display-icons-list" id="btn-' . $attribute . '">' . __('Set Icon', 'clapat_munio_plugin_text_domain') . '</a>';
        ...
        if( isset( $shortcode['sub_items'] ) && $shortcode['sub_items'] ){
            ...
            foreach ( $shortcode['sub_items'] as $subitem_name => $subitem ) {
                ...
                echo '</div>';
                echo '</div>';
                echo '<a href="#" class="btn blue add-list-item">' . sprintf(__('Add %s', 'clapat_munio_plugin_text_domain' ), $subitem['name']) . '</a>' . '<a href="#" class="btn blue remove-list-item">'. sprintf(__('Remove %s', 'clapat_munio_plugin_text_domain' ), $subitem['name']) . '</a>';
            }
        ...
        if( isset( $shortcode['has_content'] ) && $shortcode['has_content'] ){
            ...
            <div id="shortcode-content">
                ...
                <div class="label"><label id="option-label" for="shortcode-content"><?php echo __( 'Content: ', 'clapat_munio_plugin_text_domain' ); ?> </label></div>
...
if( isset( $_GET['sc'] ) ){
    ...
    require_once( MUNIO_SHORTCODES_DIR . "/include/shortcode_definitions.php" );
...
<a class="btn" id="add-shortcode"><?php _e('Add Shortcode', 'clapat_munio_plugin_text_domain'); ?></a>
